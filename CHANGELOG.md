...
- **ContentForge 3.31.0 -> 3.31.1** — an independent validation sweep re-tested the newest code
  adversarially and found the 3.31.0 newline tolerance planting the very churn it was built to
  prevent: an LF-composed multi-line replacement substituted verbatim into a CRLF body left mixed
  line endings, and fixing that exposed the twin — verify's literal containment check would then
  accuse the correctly applied fix of regression. Replacements now adopt the convention of the text
  they replace, and verify/resolve compare with the matcher's own tolerance. 4 plant-checked tests.
...
descriptions rewritten trigger-dense (the description IS the routing layer for
a model-invoked skill), written against each full SKILL.md as an honesty audit,
guarded by a new density test. DMP suite 248 -> 253. Marketplace v3.27.1 -> v3.28.0.
CF (v3.20.0) and SF (v1.18.0) unchanged.
